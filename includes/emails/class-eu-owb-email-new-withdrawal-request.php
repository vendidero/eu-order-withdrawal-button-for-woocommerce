	...
	class EU_OWB_Email_New_Withdrawal_Request extends WC_Email {
		...
		public function trigger( $order_id, $order = false ) {
			...
			if ( $order ) {
				...
				$this->placeholders['{withdrawal_date}']  = wc_format_datetime( wc_string_to_datetime( $this->object->get_meta( '_date_withdrawn' ) ) );
		...
		public function init_form_fields() {
			...
			$this->form_fields = array_merge(
				...
				array(
					'recipient' => array(
						'title'       => _x( 'Recipient(s)', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ),
						'type'        => 'text',
						/* translators: %s: WP admin email */
						'description' => sprintf( _x( 'Enter recipients (comma separated) for this email. Defaults to %s.', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ), '<code>' . esc_attr( get_option( 'admin_email' ) ) . '</code>' ),
						'placeholder' => '',
						'default'     => '',
						'desc_tip'    => true,
					),
