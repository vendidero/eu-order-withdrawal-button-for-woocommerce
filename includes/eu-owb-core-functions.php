...
/**
 * @param \Vendidero\OrderWithdrawalButton\WithdrawalOrder $order
 *
 * @return string
 */
function eu_owb_get_order_withdrawal_full_name( $order, $placeholder = false ) {
...
function eu_owb_get_order_withdrawal_date_confirmed( $order, $id = '' ) {
	...
	if ( $withdrawal = eu_owb_get_order_withdrawal( $order, $id ) ) {
		if ( 'confirmed' === $withdrawal->get_status() ) {
...
function eu_owb_get_order_withdrawal_date_rejected( $order, $id = '' ) {
	...
	if ( $withdrawal = eu_owb_get_order_withdrawal( $order, $id ) ) {
		if ( 'rejected' === $withdrawal->get_status() ) {
	...
	if ( false === $withdrawals ) {
		$withdrawals = wc_get_orders(
			array(
				...
				'status' => array( 'wc-owb-requested', 'wc-owb-confirmed', 'wc-owb-rejected' ),
				'parent' => $order_id,
				'limit'  => -1,
			)
		);
...
function eu_owb_create_order_withdrawal_request( $email, $order = false, $items = array(), $as_guest = true, $meta = array() ) {
	...
	if ( $order ) {
		if ( $existing_withdrawal = eu_owb_get_withdrawal_request( $order ) ) {
			...
			foreach ( $withdrawal->get_meta_data() as $meta ) {
				$withdrawal->delete_meta_data_by_mid( $meta->id );
			}
	...
	if ( $order ) {
		...
		if ( ! $withdrawal->get_id() ) {
			$withdrawal->set_original_status( $order->get_status() );
		}

		if ( eu_owb_custom_email_matches_order_email( $order, $email ) ) {
			$withdrawal->set_has_verified_email( true );
		}
	}
	...
	foreach ( $items as $item_id => $item_data ) {
		if ( $item = $order->get_item( $item_id, false ) ) {
			...
			$withdrawal_item->set_parent_id( $item_id );
			$withdrawal_item->set_quantity( $withdrawal_quantity );
			$withdrawal_item->set_name( $item->get_name() );
	...
	if ( ! $withdrawal_request_id ) {
		$error->add( 'invalid_request', _x( 'Error while saving the withdrawal request.', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ) );
		return $error;
	}
...
function eu_owb_order_confirm_withdrawal_request( $request_or_order ) {
	...
	if ( $order ) {
		if ( $order->has_status( $default_status ) || empty( $default_status ) ) {
			$order->add_order_note( $order_note );
			$order->save();
		} else {
...
function eu_owb_order_reject_withdrawal_request( $request_or_order, $reason = '' ) {
	...
	WC()->mailer()->emails['EU_OWB_Email_Customer_Withdrawal_Request_Rejected']->trigger( $request->get_id(), $request, $reason );
...
function eu_owb_get_email_withdrawal_items( $withdrawal, $args = array() ) {
	...
	wc_get_template(
		...
		apply_filters(
			...
			array(
				'order'         => $withdrawal->get_parent() ? $withdrawal->get_parent() : $withdrawal,
				'items'         => eu_owb_get_withdrawal_order_items( $withdrawal ),
