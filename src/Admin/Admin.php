	...
	public static function inline_order_actions( $order ) {
		if ( $request = eu_owb_get_withdrawal_request( $order ) ) {
			...
			<div class="eu-owb-withdrawal-reject-container hidden eu-owb-order-inline-edit-wrapper no-link">
				<textarea class="eu-owb-withdrawal-reject-reason" name="rejection_reason" id="rejection_reason_<?php echo esc_attr( $request->get_id() ); ?>" placeholder="<?php echo esc_attr_x( 'Reason', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ); ?>"></textarea>
								...
								foreach ( $withdrawals as $withdrawal ) :
									...
									<li class="withdrawal withdrawal-<?php echo esc_attr( Package::maybe_remove_withdrawal_order_status_prefix( $withdrawal->get_status() ) ); ?>">
										...
										<p class="meta">
											...
											<a href="<?php echo esc_url( self::get_edit_withdrawal_url( $withdrawal->get_id(), 'delete' ) ); ?>" class="delete_withdrawal" role="button"><?php echo esc_html_x( 'Delete withdrawal', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ); ?></a>
										</p>
									</li>
								<?php endforeach; ?>
							</ul>
	...
	public static function display_custom_itemmeta( $item_id, $item, $product ) {
		...
		if ( array_key_exists( $item_id, $item_map ) ) {
			if ( $item_map[ $item_id ]['confirmed'] > 0 ) {
				?>
				<mark class="withdrawal-quantity"><?php echo wp_kses_post( sprintf( _x( 'Withdrawn %1$sx', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ), $item_map[ $item_id ]['confirmed'] ) ); ?></mark>
				<?php
			}

			if ( $item_map[ $item_id ]['requested'] > 0 ) {
				?>
				<mark class="withdrawal-quantity withdrawal-requested"><?php echo wp_kses_post( sprintf( _x( 'Requested %1$sx', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ), $item_map[ $item_id ]['requested'] ) ); ?></mark>
				<?php
			}
		}
	}
