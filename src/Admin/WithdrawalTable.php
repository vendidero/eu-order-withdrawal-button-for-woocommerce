...
class WithdrawalTable extends \WP_List_Table {
	...
	public function bulk_action_notices() {
		...
		foreach ( $order_statuses as $slug => $name ) {
			if ( 'marked_' . str_replace( 'wc-owb-', '', $slug ) === $bulk_action ) { // WPCS: input var ok, CSRF ok.
				...
				$message = sprintf( _nx( '%s withdrawal status changed.', '%s withdrawal statuses changed.', $number, 'owb', 'eu-order-withdrawal-button-for-woocommerce' ), number_format_i18n( $number ) );
				break;
			}
		}
		...
		switch ( $bulk_action ) {
			case 'trashed':
				/* translators: %s: orders count */
				$message = sprintf( _nx( '%s withdrawal moved to the Trash.', '%s withdrawals moved to the Trash.', $number, 'owb', 'eu-order-withdrawal-button-for-woocommerce' ), number_format_i18n( $number ) );
				break;
			case 'untrashed':
				/* translators: %s: orders count */
				$message = sprintf( _nx( '%s withdrawal restored from the Trash.', '%s withdrawals restored from the Trash.', $number, 'owb', 'eu-order-withdrawal-button-for-woocommerce' ), number_format_i18n( $number ) );
				break;
			case 'deleted':
				/* translators: %s: orders count */
				$message = sprintf( _nx( '%s withdrawal permanently deleted.', '%s withdrawals permanently deleted.', $number, 'owb', 'eu-order-withdrawal-button-for-woocommerce' ), number_format_i18n( $number ) );
				break;
		}
	...
	public function display() {
		...
		if ( ! empty( $this->order_query_args['s'] ) ) {
			...
			/* translators: %s: Search query. */
				_x( 'Search results for: %s', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ),
		...
		$this->search_box( esc_html_x( 'Search withdrawals', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ), 'withdrawals-search-input' );
	...
	public function render_blank_state() {
		...
		<div class="woocommerce-BlankState">
			<h2 class="woocommerce-BlankState-message">
				<?php echo esc_html_x( 'When you receive a new withdrawal, it will appear here.', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ); ?>
			</h2>
		</div>
		<?php
	}
	...
	protected function get_bulk_actions() {
		...
		if ( array( 'trash' ) === $selected_status ) {
			$actions = array(
				'untrash' => _x( 'Restore', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ),
				'delete'  => _x( 'Delete permanently', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ),
			);
		} else {
			$actions = array(
				'mark_confirmed' => _x( 'Confirm', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ),
				'mark_rejected'  => _x( 'Reject', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ),
				'trash'          => _x( 'Move to Trash', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ),
			);
		}
	...
	public function get_views() {
		...
		$view_links['all'] = $this->get_view_link( 'all', _x( 'All', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ), $all_count, '' === $current || 'all' === $current );
	...
	public function render_withdrawal_column( $order ) {
		echo '<mark class="order-status status-owb-type-' . esc_attr( $order->is_partial() ? 'partial' : 'full' ) . '"><span>' . sprintf( $order->is_partial() ? esc_html_x( 'Partial %s', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ) : esc_html_x( 'Full %s', 'owb', 'eu-order-withdrawal-button-for-woocommerce' ), esc_html( $order->get_id() ) ) . '</span></mark>';
	}
	...
	public function render_order_date_column( $order ) {
		...
		if ( $order_timestamp > strtotime( '-1 day', time() ) && $order_timestamp <= time() ) {
			...
			/* translators: %s: human-readable time difference */
				_x( '%s ago', 'owb-human-readable-time-diff', 'eu-order-withdrawal-button-for-woocommerce' ),
